"""Run request/reset safety contracts on a disposable PostgreSQL primary."""
...
from tests import test_runtime_safety as contracts


@unittest.skipUnless(os.getenv("REVGUARD_TEST_POSTGRES_DSN"), "requires disposable PostgreSQL server")
class TestPostgresRuntimeSafety(contracts.TestRuntimeSafety):
    def setUp(self):
        import psycopg
        ...
        database = "revguard_runtime_" + uuid.uuid4().hex
        ...
        with patch.object(contracts, "Store", side_effect=lambda _: PostgresStore(dsn)):
            super().setUp()
