"""Run the workflow concurrency contracts against unique PostgreSQL databases."""
...
from tests import test_workflow_concurrency as contracts


@unittest.skipUnless(os.getenv("REVGUARD_TEST_POSTGRES_DSN"), "requires disposable PostgreSQL server")
class TestPostgresWorkflowConcurrency(contracts.TestWorkflowConcurrency):
    def setUp(self):
        import psycopg
        ...
        database = "revguard_workflow_" + uuid.uuid4().hex
        ...
        with patch.object(contracts.fixtures, "Store", side_effect=lambda _: PostgresStore(make_conninfo(admin, dbname=database))):
            super().setUp()
