...
2026-09-18 两次官网内容更新后（「06 · 可验证成果」测试口径 353 → 360 → **369 项（286 默认 + 83 集成）**，「07 · 开源复用」新增六行业迁移矩阵）分别重新推送 `main` 并复跑同一套探针，公网**均为 12 项全绿**：`release=0.6.0-rc3`，CASE-2026-0001 渲染 10 步 / 追踪 53 行，CASE-2026-0008 渲染 11 步 / 追踪 60 行，两个成片直链第 1 次尝试即返回 200，无失败请求与浏览器 SEVERE 日志；结果与截图覆盖 [`public/`](public/)。
