...
try:
    driver.get(os.environ.get('DEMO_URL','http://revguard-grafana-api-preview:9000/demo/?view=observability'))
    ...
    if frames:
        ...
        # Grafana 只在面板进入视口时才发起查询：滚动一遍确保 23 个面板全部被请求，
        # 否则断言会因懒加载只统计到首屏面板而误判。
        offset=0
        while offset < driver.execute_script('return document.body.scrollHeight'):
    ...
    expected=int(os.environ.get('EXPECTED_PANELS','23'))
    result['expected_panels']=expected
    result['passed']=len(frames)==1 and result['unique_panel_ids']==list(range(1,expected+1)) and all(x['status']==200 for x in queries) and result.get('frame_text','').count('正常')>=3 and not failed and not result['console']
