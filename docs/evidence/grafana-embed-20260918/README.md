...
彩排栈入口：`http://10.10.10.202:19088/demo/?view=observability`
常驻栈入口：`http://10.10.10.202:19000/demo/?view=observability`

## 本次修复的两个问题

1. **彩排栈此前没有嵌入 Grafana**：`revguard-api-dev` 容器创建于 `docker-compose.finals.yml` 增加 Grafana 覆盖之前，缺少 token 挂载与 `observability` 网络。用
...
同一次会话的彩排栈冒烟结果为 **6 PASS / 0 FAIL**（双案例终态、ERPNext 证据来源、真人身份验证跨度均通过），现场 Demo 通路可用。
