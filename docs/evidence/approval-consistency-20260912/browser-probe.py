...
root = 'http://revguard-approval-preview:9000'
...
try:
    ...
    if phase == 'recovery':
        button = wait.until(lambda d: d.find_element(By.XPATH, "//button[contains(.,'核对并恢复执行')]"))
        assert before['case']['status'] == 'READY_TO_EXECUTE'
        assert not before['executions']
        driver.save_screenshot(str(out/'recovery-entry.png'))
    else:
        button = wait.until(lambda d: d.find_element(By.CLASS_NAME, 'primary-action'))
        wait.until(lambda d: '批准' in button.text and button.is_enabled())
    button.click()
    ...
    if phase == 'failure':
        wait.until(lambda d: '审批提交结果未确认' in d.find_element(By.TAG_NAME,'body').text)
        after = dashboard()
        assert after['case'] == before['case']
        assert after['approval']['status'] == 'PENDING'
        assert after['executions'] == []
        assert driver.find_element(By.CSS_SELECTOR,'input[type="password"]').get_attribute('value') == ''
        result.update(error_visible=True, case_preserved=True, approval_still_pending=True, no_money_effect=True)
        driver.save_screenshot(str(out/'approval-failure.png'))
    else:
        ...
        assert len(after['agent_tasks']) == 16
        assert after['case']['team_run']['status'] == 'COMPLETED'
        assert all(t['status'] == 'SUCCEEDED' for t in after['agent_tasks'])
        result.update(final_status='CLOSED', executions=2, real_mcp_tasks=16)
