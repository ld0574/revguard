...
revguard-intake ──► revguard-evidence ──► revguard-policy ──► revguard-calculation
   │ （实体消歧）      （并行采集 8 类证据）   （版本 Time Travel）   （规则引擎复算）
   │                                                              │
   ▼                                                              ▼
...
| 3 | revguard-evidence | 跨系统并行采集证据，构建 Evidence Package | partner、order_id、case_id | evidence[]（含 tool_receipt）、evidence_score、evidence_gaps | 8 个只读工具（crm/contract/policy/finance） | score ≥ 0.6 → 移交 Policy；< 0.6 → 挂起补证升级；只读权限，不得脑补数据 |
...
| 8 | revguard-executor | **唯一**允许触碰资金台账的受控执行 Agent | 已审批 Action Plan（approval_token + 幂等键） | action_id、执行前后快照、rollback_token | `commission.create_adjustment_draft` / `submit_adjustment` / `reverse_adjustment` | 六项前置条件全满足才动作；无凭证执行会被拒绝并审计；不得自行宣布成功（ADR-002） |
| 9 | revguard-verifier | 执行后独立重新查询源系统复核 | order_id、expected_components | verification_status、component_checks、rollback_required | `finance.get_commission_ledger`（独立新查询） | 不复用 Executor 回执作证据；任一偏差 → FAILED + rollback_required |
| 10 | revguard-knowledge | 沉淀 Case Memory、生成评测样本与回复草稿 | 完整案件轨迹 | GOLDEN/BAD 样本、审计报告、回复草稿 | `mail.create_reply_draft` / `ticket.update_case` | 只生成草稿不直接发送；证据链不完整标 BAD；沉淀动作本身入审计 |

## 三条不可逾越的全局边界（ADR）

1. **ADR-001**：LLM 不计算金额。所有金额由 Decimal 规则引擎产出，Agent 只组装 facts 与解释结果。
2. **ADR-002**：Executor 与 Verifier 分离。执行成功与否由独立查询判定，不由执行方自证。
3. **写权限最小化**：全平台仅 revguard-executor 持有写工具 scope，且必须携带审批凭证 + 幂等键。

## Trace 约定

- 每次工具调用携带 `case_id` + `actor` + 最小 `scope`，产生 `tool_receipt`（RCPT- 前缀）；
