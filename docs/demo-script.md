...
页面固定演示 `CASE-2026-0008`。它使用合成业务数据，但 AgentTeams/Matrix 调度、StageTask、状态迁移、
...
| 04:35–05:10 | 打开完整审计/工程证据页签 | “同一个 case ID 下保留 20 个成功 StageTask、9 个 Worker、16 种 Skill，以及 room、message、task、request、receipt、Trace 和审计事件。凭证会被替换为不可授权指纹，金额、审批、执行与回滚可逐项对账。” | 20/20；9 Workers；`AgentTeams Matrix`；Audit/Trace；无原始 token |
