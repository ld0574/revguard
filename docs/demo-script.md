...
8. **评测与复现（50 秒）**：运行 `make verify`，展示 64 项测试和 102/102 场景评测；
...
| 评审关注 | 演示证据 |
|---|---|
| AgentTeams 协同 | 1 Manager + 9 Worker SOUL、结构化 Case State、角色边界 |
...
| 权限边界 | forged/cross-case/expired/over-amount/scope-escalation 6 个攻击探针 |
| 受控写入 | signed token + gross quota + idempotency + snapshot + receipt |
...
- AgentTeams LLM 不计算金额，也不持有 API key；Bearer key 由 Secret/Adapter 注入。
