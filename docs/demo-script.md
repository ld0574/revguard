...
录制环境打开 `http://10.10.10.202:19000/demo/`。先点“重置录制演示”，确认状态回到
`CREATED`，再开始整段录制；不要在镜头里展示 `.env`、容器环境变量或 Element 登录信息。

主视频默认演示 `CASE-2026-0008`，页面也可通过案件下拉或 `?case=` 切换其他 Golden Case。
它使用合成业务数据，但 AgentTeams/Matrix 调度、StageTask、状态迁移、
...
| 00:55–01:15 | 点击“启动多 Agent 调查” | “现在启动的不是预置动画。Orchestrator 先在 AgentTeams Team 房间确认控制面，再根据案件状态创建版本绑定的 StageTask；任务经 Matrix 派到各 Worker 独立房间，避免聊天历史互相污染。” | `AGENTTEAMS LIVE`；Orchestrator `ACKNOWLEDGED`；状态从 CREATED 开始推进 |
| 01:15–02:00 | 展开 Agent 任务账本中的 2–3 项，再查看证据表 | “每个 Worker 都收到服务端固化的输入。这里能逐项展开真实输入、StageResult 输出，并用 room、message、request、task、receipt 和 trace 六类标识对账。聊天文字不能推进状态，只有服务端落库的 SUCCEEDED StageResult 才能继续。” | Intake/Evidence/Policy 的输入输出；独立 room；六类关联标识；8/8 证据与 hash/receipt |
...
| 03:10–03:25 | 组员亲自点击“批准并执行 14,400 KES” | “现在由独立 Approver 做出明确决定。这个动作会写入 `simulated_human=false` 的审计事件，然后 AgentTeams 从持久化状态继续，而不是重新跑一条脚本。” | 人手点击录入；按钮 loading；状态进入 READY/EXECUTING |
...
| 04:35–05:10 | 打开完整审计/工程证据页签 | “同一个 case ID 下保留 20 个成功 StageTask、9 个 Worker、16 种 Skill，以及 room、message、task、request、receipt、Trace 和审计事件。AGENT 行是 Matrix 与 Worker 的真实端到端秒级耗时；SKILL/TOOL 行是进程内确定性执行，低于一毫秒会明确显示 `<1ms`。” | 20/20；9 Workers；63 spans；AGENT 秒级耗时；无原始 token |
...
| 05:35–05:50 | 下移到公式与边界，再切工程证据 | “这不是现金节省承诺。页面同时展示公式、8 个合成样本和 NOT ALLOWED 生产声明；接入企业实测基线后才能对外主张收益。当前持久层运行官方开源 PolarDB-PG 15 local_instance；云 PolarDB 高可用与 PITR 仍明确标记为待验收。” | 公式；SYNTHETIC；Open-source PolarDB-PG PASSED；Cloud HA/PITR PENDING |
| 05:50–06:00 | 回到全景，停在闭环流水线 | “RevGuard 的差异化不是 Agent 数量，而是给 Agent 协作加上可执行的治理底座：真实 AgentTeams 调度、服务端最小权限、真实状态流、人审不可自签、执行与验证分离、失败可回滚、价值可测算。” | 完整 8 段流水线与 ROLLED_BACK/PASSED |
...
- 可以说“已部署官方开源 PolarDB-PG 15 local_instance 并完成链路验收”；不能说“已完成云上 PolarDB 高可用、备份或 PITR 验收”。
- 不把本地 MCP harness 与录制服务器的 AgentTeams/Matrix 链路混为一谈；只对驾驶舱中已落库的 Matrix event 和 StageResult 作陈述。
