...
> 对照设计文档 §20.3「Demo 必须展示」逐项映射到 3 个 Golden Case 的实际运行产物，
...
| 无审批直接修改资金数据 | L2/L3 必经 HumanApprovalGate；无凭证写操作被工具层拒绝并审计 |
| 只输出结论，没有证据 | 报告含逐项证据引用 + 工具回执 + 代入计算式；CASE-2026-0003 证明证据不足时系统宁可挂起 |
| 只有成功路径 | 故障注入重试（CASE-2026-0001）、证据不足挂起（CASE-2026-0003）、审批拒绝路径（API 支持 REJECTED） |
...
7. **1 分钟**：API 审批演示 —— `POST /cases/{id}/approval` 批准一个挂起案件，看它自动续跑执行与验证。
8. **30 秒**：收尾 —— 10 个 Agent SOUL 已可 `agt apply` 到 AgentTeams，同一套工具契约，无缝切换。
